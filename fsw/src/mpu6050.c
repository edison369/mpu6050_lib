...
int sensor_mpu6050_ioctl(i2c_dev *dev, ioctl_command_t command, void *arg){
  ...
  switch (command) {
    ...
    case SENSOR_MPU6050_SET_REG:
      err = sensor_mpu6050_set_reg_8(dev, SENSOR_MPU6050_Priv_Data.register.address, SENSOR_MPU6050_Priv_Data.register.value);
      break;
...
int sensor_mpu6050_set_register(int fd, uint8_t register, uint8_t val){
  SENSOR_MPU6050_Priv_Data.register.address = register;
  SENSOR_MPU6050_Priv_Data.register.value = val;
